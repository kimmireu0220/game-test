-- 라운드 종료 시 created_at + 정답 목록을 한 번에 반환 (클라이언트 요청 1회로 해결)
create or replace function public.get_updown_round_result(p_round_id uuid)
returns table(created_at timestamptz, correct_list jsonb)
language sql
security definer
...
comment on function public.get_updown_round_result(uuid) is '업다운 라운드 결과: created_at + 정답 목록(correct_at 순) 한 번에 반환';
