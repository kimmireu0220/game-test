// Supabase Edge Function: start-number-order-round
// 호스트만 호출. no_rounds에 start_at = now()+4초 insert.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: round, error: insertRoundError } = await supabase
      .from("no_rounds")
      .insert({ room_id, start_at: startAt })
      .select("id, start_at")
      .single();
    ...
    return new Response(
      JSON.stringify({ id: round.id, start_at: round.start_at }),
